...
def main():
    ...
    members_data: List[dict] = data.get("members", [])

    # id -> list name map
    list_name_by_id = {lst["id"]: lst.get("name", "") for lst in lists_data}

    # id -> member name map (using fullName, fallback to username)
    member_name_by_id = {
        m["id"]: m.get("fullName") or m.get("username", "")
    ...
    for card in cards:
        ...
        # members (comma-separated list of names)
        member_ids = card.get("idMembers", [])
        members = ", ".join(
            member_name_by_id.get(mid, mid) for mid in member_ids
        )
